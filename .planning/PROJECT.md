...
**Goal:** Build a Twitter-native cascade trained on TwiBot-20 to show the cascade performs meaningfully when platform-matched, while removing the ineffective online novelty-recalibration path from the Reddit transfer system.

**Delivered features:**
- Twitter-native feature extraction for all three stages with no Reddit mappings, imputing, or zero-fill stand-ins
- Full TwiBot-native cascade training on `train.json` and evaluation on `test.json`
- Paper comparison outputs that contrast Reddit-trained transfer against the TwiBot-native baseline
- Removal of online novelty recalibration from the maintained Reddit-transfer contract

## Current State

v1.4 shipped 2026-04-18. The active Reddit model is `trained_system_v12.joblib`, and the separate TwiBot-native model is `trained_system_twibot20.joblib`. The maintained comparison flow now contrasts the Reddit transfer baseline against the native TwiBot baseline and generates Table 5 from that contract. Online novelty recalibration is retained only as historical evidence and is no longer part of the maintained release path.

Known gaps carried forward:
- Full pytest green-suite is still blocked in this Windows workspace by temp-dir cleanup permissions; production code verification used `py_compile` plus direct smoke checks instead
- Stale pre-Phase-12 TwiBot artifacts remain at repo root and are superseded by the maintained phase artifact directories
...
- Maintain separate Reddit-transfer and TwiBot-native artifacts with a paper-facing comparison path
...
---
*Last updated: 2026-04-18 - v1.4 milestone shipped (Twitter-Native Supervised Baseline)*
