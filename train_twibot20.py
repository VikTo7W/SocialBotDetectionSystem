...
from botdetector_pipeline import FeatureConfig, StageThresholds, predict_system, train_system
from calibrate import write_calibration_report_artifact, calibrate_thresholds
from evaluate import evaluate_s3
from features_stage1 import extract_stage1_matrix as _orig_stage1_extract
from features_stage2 import extract_stage2_features as _orig_stage2_extract
...
SEED = 42
DEFAULT_NATIVE_MODEL_PATH = "trained_system_twibot20.joblib"
...
def ensure_safe_model_output_path(path: str) -> str:
    ...
    if basename in _PROTECTED_MODEL_ARTIFACTS:
        raise ValueError(
            f"Refusing to overwrite protected model artifact: {basename}"
        )
    return path


@contextmanager
def native_feature_overrides():
    old_s1 = bp.extract_stage1_matrix
    old_s2 = bp.extract_stage2_features
    bp.extract_stage1_matrix = extract_stage1_matrix_twitter
    bp.extract_stage2_features = extract_stage2_features_twitter
    try:
        yield
    finally:
        bp.extract_stage1_matrix = old_s1
        bp.extract_stage2_features = old_s2
...
def train_twibot20(
    ...
    model_output_path: str = DEFAULT_NATIVE_MODEL_PATH,
    output_dir: str = PHASE15_ARTIFACT_DIR,
    seed: int = SEED,
    calibrate_trials: int = 8,
) -> dict:
...
if __name__ == "__main__":
    ...
    model_output_path = sys.argv[4] if len(sys.argv) > 4 else DEFAULT_NATIVE_MODEL_PATH
    ...
    print(f"[twibot20-native] calibration: {summary['paths']['calibration']}")
